"""
evaluate the inverse function with three new metrics:
    distance: the euclidean distance between original data and inversed data
    accuracy: the prediction accuracy between original data and inversed data
    confidence value: the predicted confidence value of original data and inversed data

we use three types of data to evaluate inverse function:
    successfully attack adversarial samples
    failed attack adversarial samples
    true samples from real distribution
"""
...
if __name__ == "__main__":
    ...
    train_new_confidence = deepview.model(train_recon).max(axis=1)
    ...
    adv_succ_new_confidence = deepview.model(adv_succ_recon).max(axis=1)
    ...
    adv_fail_new_confidence = deepview.model(adv_fail_recon).max(axis=1)
    ...
    true_new_confidence = deepview.model(true_recon).max(axis=1)
